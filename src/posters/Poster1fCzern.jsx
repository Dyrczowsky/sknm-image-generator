...
// 1f · REKRUTACJA — wariant czerń (czerń u góry, złoto na dole - najwyższy
// kontrast złota)
export function Poster1fCzern({ data }) {
  ...
  return (
    <PosterFrame background={colors.black} color={colors.cream} padding={72}>
      <div
        style={{
          position: 'absolute', bottom: 0, left: 0, right: 0, height: 340, background: colors.gold,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 26, position: 'relative', maxWidth: 900 }}>
        ...
        <div style={{ fontSize: 38, fontWeight: 600, lineHeight: 1.3, color: colors.creamMuted }}>
          {subtitle || 'Seminaria, konkursy, wyjazdy i własne projekty badawcze. Każdy rok studiów, każdy wydział.'}
        </div>
      </div>

      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 32, position: 'relative', color: colors.ink }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 8 }}>
          <Badge color={colors.black} style={{ font: `700 26px ${fontMono}`, letterSpacing: '.1em' }}>{badge || 'SPOTKANIE ORGANIZACYJNE'}</Badge>
        ...
        <LogoRow alignItems="flex-end">
          <PlaceholderBox label="kod QR" width={130} height={130} style={{ borderColor: 'rgba(18,18,18,.4)', color: 'rgba(18,18,18,.6)' }} />
