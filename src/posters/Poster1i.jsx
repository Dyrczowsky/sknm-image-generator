...
// 1i · WYKŁAD — negatyw na czerni
export function Poster1i({ data }) {
  const { title, subtitle, speaker, event_date, event_time, location, badge, logos } = withPlaceholders(data)

  return (
    <PosterFrame background={colors.black} color={colors.cream} padding={72}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 28, position: 'relative', zIndex: 1 }}>
        <Badge background={colors.gold} color={colors.cream} style={{ fontSize: 24 }}>{badge || 'WYKŁAD OTWARTY'}</Badge>
        <div style={{ fontSize: 120, fontWeight: 800, lineHeight: 0.94, letterSpacing: '-.035em', fontKerning: 'none' }}>
          {title}
        </div>
        <div style={{ fontSize: 36, fontWeight: 600, color: colors.gold }}>{speaker}</div>
      </div>
      ...
      <div style={{ position: 'absolute', top: 0, right: 0, width: 700, height: 600, background: 'rgba(255,255,255,.04)', clipPath: 'polygon(0 0,100% 0,100% 100%)' }} />
      <div style={{ position: 'absolute', bottom: 0, right: 0, width: 920, height: 780, background: '#1E1E1E', opacity: 0.42, clipPath: 'polygon(100% 0,100% 100%,0 100%)' }} />
      <div style={{ position: 'absolute', bottom: 0, left: 0, width: 520, height: 300, background: '#0A0A0A', clipPath: 'polygon(0 100%,0 0,100% 100%)' }} />
      <div style={{ position: 'absolute', left: 18, bottom: 72, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ width: 38, height: 32, background: colors.gold, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
        <div style={{ width: 38, height: 32, background: colors.gold, clipPath: 'polygon(0 0,100% 0,50% 100%)', opacity: 0.66 }} />
        <div style={{ width: 38, height: 32, background: colors.gold, clipPath: 'polygon(0 0,100% 0,50% 100%)', opacity: 0.33 }} />
      </div>
    </PosterFrame>
  )
}
