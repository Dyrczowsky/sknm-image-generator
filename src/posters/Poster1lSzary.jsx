...
// 1l · OGŁOSZENIE — wariant szary monochrom
export function Poster1lSzary({ data }) {
  const { title, subtitle, logos } = withPlaceholders(data)

  return (
    <PosterFrame background={colors.paper} color={colors.slate} padding={96}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center', gap: 32 }}>
        <div style={{ display: 'flex', gap: 10 }}>
          <div style={{ width: 46, height: 40, background: colors.grayDark, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
          <div style={{ width: 46, height: 40, background: colors.grayDark, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
        </div>
        ...
        {subtitle && (
          <div style={{ font: `700 24px ${fontMono}`, letterSpacing: '.1em', color: colors.grayDark }}>— {subtitle.toUpperCase()}</div>
        )}
      </div>
