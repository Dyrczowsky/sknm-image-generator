...
// 1d · DATA — wariant czerń (negatyw na czerni, złote akcenty)
export function Poster1dCzern({ data }) {
    ...
    <PosterFrame background={colors.black} color={colors.cream} padding={72}>
      ...
      <div style={{ display: 'flex', gap: 36, alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <div style={{ fontSize: 60, fontWeight: 800, lineHeight: 1, letterSpacing: '-.02em', fontKerning: 'none' }}>
            {title}
          </div>
          <InfoLine parts={[subtitle]} secondLine={location} style={{ color: colors.creamMuted, maxWidth: '24ch' }} />
        </div>
      </div>

      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
        <div style={{ display: 'flex', gap: 6 }}>
          <div style={{ width: 56, height: 44, background: colors.gold, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
          <div style={{ width: 56, height: 44, background: colors.coral, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
          <div style={{ width: 56, height: 44, background: colors.cream, clipPath: 'polygon(0 0,100% 0,50% 100%)' }} />
        </div>
        <LogoRow gap={14}>
          <LogoSlot logo={logos.pk} variant="dark" width={180} height={68} />
          <LogoSlot logo={logos.faculty} variant="dark" width={180} height={68} />
        </LogoRow>
      </div>
    </PosterFrame>
  )
