...
// 1b · GOŚĆ — wariant złoto (złoto na granacie - złoto na jasnym tle ma za
// niski kontrast, patrz theme.js/blocks README)
export function Poster1bZloto({ data }) {
  ...
  return (
    <PosterFrame background={colors.navy} color={colors.cream}>
      <PhotoGallery photos={photos.photo} label={<>zdjęcie prelegenta<br />1080 × 600</>} style={{ height: 600 }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: 420, height: 420, background: colors.gold, clipPath: 'polygon(0 0,100% 0,0 100%)', display: 'flex', padding: '44px 0 0 44px', boxSizing: 'border-box' }}>
          <img src={sygnetZloty} alt="SKNM" style={{ width: 132, height: 132, objectFit: 'contain' }} />
        </div>
      </PhotoGallery>
      ...
      <div style={{ flex: 1, padding: '56px 72px 72px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 820 }}>
          ...
          <InfoLine parts={[speaker]} secondLine={location} style={{ fontSize: 32, color: colors.creamMuted, lineHeight: 1.35 }} />
        </div>

        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
          <div style={{ fontSize: 22, fontWeight: 600, color: colors.gold }}>Wstęp wolny · sknm.pk.edu.pl</div>
          <LogoRow>
            <LogoSlot logo={logos.pk} variant="dark" width={190} height={72} />
            <LogoSlot logo={logos.faculty} variant="dark" width={190} height={72} />
          </LogoRow>
        </div>
      </div>
    </PosterFrame>
