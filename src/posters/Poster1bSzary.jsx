...
// 1b · GOŚĆ — wariant szary monochrom
export function Poster1bSzary({ data }) {
  ...
  return (
    <PosterFrame background={colors.paper} color={colors.slate}>
      <PhotoGallery photos={photos.photo} label={<>zdjęcie prelegenta<br />1080 × 600</>} style={{ height: 600 }}>
        <div style={{ position: 'absolute', top: 0, left: 0, width: 420, height: 420, background: colors.grayDark, clipPath: 'polygon(0 0,100% 0,0 100%)', display: 'flex', padding: '44px 0 0 44px', boxSizing: 'border-box' }}>
      ...
      <div style={{ flex: 1, padding: '56px 72px 72px', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 820 }}>
          <Badge color={colors.grayDark}>{badge || 'SEMINARIUM SKNM'}</Badge>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
          <div style={{ fontSize: 22, fontWeight: 600, color: colors.grayDark }}>Wstęp wolny · sknm.pk.edu.pl</div>
          <LogoRow>
