...
// 1f · REKRUTACJA — wariant złoto (złoto u góry z ciemnym tekstem, granat na dole)
export function Poster1fZloto({ data }) {
  const { title, subtitle, event_date, event_time, location, badge, logos } = withPlaceholders(data)

  return (
    <PosterFrame background={colors.gold} color={colors.ink} padding={72}>
      <div
        style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 32, position: 'relative', color: colors.cream }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 8 }}>
          <Badge color={colors.gold} style={{ font: `700 26px ${fontMono}`, letterSpacing: '.1em' }}>{badge || 'SPOTKANIE ORGANIZACYJNE'}</Badge>
